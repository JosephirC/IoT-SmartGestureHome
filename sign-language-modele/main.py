...
s2p_map = {k.lower(): v for k, v in load_json_file(
    "src/sign_to_prediction_index_map.json").items()}
p2s_map = {v: k for k, v in load_json_file(
    "src/sign_to_prediction_index_map.json").items()}


def encoder(x): return s2p_map.get(x.lower())
def decoder(x): return p2s_map.get(x)
...
def real_time_asl():
    ...
    with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
        ...
        while cap.isOpened():
            ...
            white_column = np.ones(
                (height // 8, width, 3), dtype='uint8') * 255
            ...
            cv2.resizeWindow('Webcam Feed', 1024, 960)  # largeur, hauteur
